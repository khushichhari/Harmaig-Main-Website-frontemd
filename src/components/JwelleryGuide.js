import React from 'react';
import './Home.css'; // Import the external CSS file

const JewelleryGuide = () => {
  const handleFindSize = () => {
    // URL of the PDF file
    const pdfUrl = 'https://docs.google.com/document/d/1T5rTeENgq0ak8Bvvp-AIqD1qBD0lQ0INUUf4TIFS2FM/edit?tab=t.0'; // Update this path to your PDF file location
    window.open(pdfUrl, '_blank'); // Open PDF in a new tab
  };

  return (
    <>
      <div className="guide-container">
        <h1>Jewellery Size Guide</h1>
        <p>Finding the perfect fit for your jewellery is important. Follow our guide to get the right size for your rings, bracelets, and more!</p>
        <button className="find-size-button" onClick={handleFindSize}>
          Find Your Jewellery Size
        </button>
      </div>

      <div id="line">
        <img src="data:image/png;base64," alt="..."/>
      </div>
    </>
  );
};

export default JewelleryGuide;
